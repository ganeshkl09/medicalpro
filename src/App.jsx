...
import { LoginScreen } from './TwoLetterLogin.jsx';
import CryptoJS from 'crypto-js';

/* ── DATA PERSISTENCE SERVICE ──────────────────────────────────────────────── */
// Using localStorage for web, for Windows electron-switch to electron-store
const DATA_KEYS = {
  medicines: 'medstore_medicines',
  suppliers: 'medstore_suppliers',
  customers: 'medstore_customers',
  employees: 'medstore_employees',
  sales: 'medstore_sales',
  pos: 'medstore_pos',
  settings: 'medstore_settings',
  loggedIn: 'medstore_loggedIn'
};

// CHANGE THIS SECRET KEY - use your own random string!
const SECRET_KEY = import.meta.env.VITE_SECRET_KEY || "MedStore@2024#SecureKey!";
...
/* ── INITIAL DATA ───────────────────────────────────────────────────────────── */
const CHART_DATA = [
  { month: "Nov", sales: 48200, profit: 18400 },
  { month: "Dec", sales: 62100, profit: 24800 },
  { month: "Jan", sales: 58400, profit: 22900 },
  { month: "Feb", sales: 69700, profit: 28100 },
  { month: "Mar", sales: 73400, profit: 29600 },
  { month: "Apr", sales: 81200, profit: 32400 },
  { month: "May", sales: 90300, profit: 36700 },
];
const CAT_DATA = [
...
const statusBadge = (s) => {
  const m = {
    Received: { bg: C.successBg, color: C.success, border: C.successBorder },
    ...
    <div style={{ width: open ? 220 : 60, background: C.sidebar, display: "flex", flexDirection: "column", height: "100vh", transition: "width 0.2s", flexShrink: 0, overflow: "hidden", zIndex: 100 }}>
      ...
      <div style={{ padding: "12px", borderTop: `1px solid ${C.sidebarHover}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {open && <div><div style={{ color: "#fff", fontSize: 12, fontWeight: 600 }}>{ownerName || "Admin"}</div><div style={{ color: "#64748B", fontSize: 11 }}>Manager</div></div>}
        </div>
      </div>
    </div>
  );
...
function PurchaseOrders({ pos, setPos, suppliers, medicines, setMedicines }) {
  ...
  const [form, setForm] = useState({ supplier: "", expected: today(), items: [{ name: "", batch: "", qty: "", price: "", mrp: "", gst: 5, expiry: "" }] });
  const [sel, setSel] = useState(null);

  const addItem = () => setForm(f => ({ ...f, items: [...f.items, { name: "", batch: "", qty: "", price: "", mrp: "", gst: 5, expiry: "" }] }));
        ...
        <Modal title="New Purchase Order" onClose={() => setModal(false)} width={820}>
          ...
          {form.items.map((it, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "2.2fr 0.85fr 0.6fr 0.55fr 0.8fr 0.8fr 0.65fr 30px", gap: 6, marginBottom: 8, alignItems: "center" }}>
              <input value={it.name} onChange={e => updateItem(i, "name", e.target.value)} placeholder="Medicine name" style={sInp} />
              <input value={it.batch || ""} onChange={e => updateItem(i, "batch", e.target.value)} placeholder="Batch No." style={sInp} />
              <input value={it.expiry || ""} onChange={e => updateItem(i, "expiry", e.target.value)} placeholder="MM/YY" style={sInp} />
              <input type="number" value={it.qty} onChange={e => updateItem(i, "qty", e.target.value)} placeholder="Qty" style={sInp} />
              <input type="number" value={it.price} onChange={e => updateItem(i, "price", e.target.value)} placeholder="Cost" style={sInp} />
              <input type="number" value={it.mrp || ""} onChange={e => updateItem(i, "mrp", e.target.value)} placeholder="MRP" style={sInp} />
              <select value={it.gst != null ? it.gst : 5} onChange={e => updateItem(i, "gst", +e.target.value)} style={{ ...sInp, fontWeight: 600, color: C.info, background: C.infoBg }}>
                {GST_RATES.map(r => <option key={r} value={r}>{r}%</option>)}
              </select>
              <button onClick={() => removeItem(i)} style={{ background: C.dangerBg, color: C.danger, border: `1px solid ${C.dangerBorder}`, borderRadius: 6, padding: 0, cursor: "pointer", width: 30, height: 30 }}><Trash2 size={12} /></button>
            </div>
          ))}
      ...
      {sel && (
        <Modal title={`PO Details: ${sel.id}`} onClose={() => setSel(null)} width={700}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 12 }}>
              <thead><tr>{["Medicine", "Batch", "Expiry", "Qty", "Cost Price", "GST %", "GST Amt", "MRP", "Line Total"].map(h => <th key={h} style={sTh}>{h}</th>)}</tr></thead>
              <tbody>{sel.items.map((it, i) => {
    ...
    <div>
      ...
      <div style={sCard}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr>
              {["Customer", "Phone", "Email", "Address", "Loyalty Points", "Total Purchases", "Actions"].map(h => <th key={h} style={sTh}>{h}</th>)}
            </tr>
          </thead>
    ...
    <div>
      ...
      <div style={sCard}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr>
              {["Medicine", "Category", "Batch", "Expiry Date", "Days Left", "Stock", "Status"].map(h => <th key={h} style={sTh}>{h}</th>)}
            </tr>
          </thead>
          <tbody>{filtered.map(m => {
...
function AppSettings({ settings, setSettings, exportBackup, importBackup }) {
  ...
  const save = () => {
    saveToStorage(DATA_KEYS.settings, store);
    setSettings(store);
    setSaved(true);
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={sCard}>
          ...
          <Field label="Store Name"><input value={store.name || ""} onChange={e => setStore(s => ({ ...s, name: e.target.value }))} style={sInp} readOnly disabled /></Field>
          <Field label="Owner/Manager"><input value={store.owner || ""} onChange={e => setStore(s => ({ ...s, owner: e.target.value }))} style={sInp} readOnly disabled /></Field>
          <Field label="Address"><textarea value={store.addr || ""} onChange={e => setStore(s => ({ ...s, addr: e.target.value }))} style={{ ...sInp, resize: "vertical", minHeight: 60 }} readOnly disabled /></Field>
          <Field label="Phone"><input value={store.phone || ""} onChange={e => setStore(s => ({ ...s, phone: e.target.value }))} style={sInp} readOnly disabled /></Field>
          <Field label="Email"><input value={store.email || ""} onChange={e => setStore(s => ({ ...s, email: e.target.value }))} style={sInp} readOnly disabled /></Field>
        </div>
        <div>
          <div style={{ ...sCard, marginBottom: 16 }}>
            ...
            <Field label="GST Number"><input value={store.gst || ""} onChange={e => setStore(s => ({ ...s, gst: e.target.value }))} style={sInp} readOnly disabled /></Field>
            <Field label="Drug License No."><input value={store.license || ""} onChange={e => setStore(s => ({ ...s, license: e.target.value }))} style={sInp} readOnly disabled /></Field>
            <Field label="DL No."><input value={store.dl || ""} onChange={e => setStore(s => ({ ...s, dl: e.target.value }))} style={sInp} readOnly disabled /></Field>
            <Field label="Login Code (2 letters)">
  <input 
    value={store.loginCode || ""} 
    onChange={(e) => {
      let val = e.target.value.toUpperCase();
      if (val.length <= 2) {
        setStore(prev => ({ ...prev, loginCode: val }));
      }
    }} 
    style={sInp} 
    maxLength={2} 
...
export default function App() {
  ...
  const [medicines, setMedicines] = useState(() => loadFromStorage(DATA_KEYS.medicines, []));
  const [suppliers, setSuppliers] = useState(() => loadFromStorage(DATA_KEYS.suppliers, []));
  const [customers, setCustomers] = useState(() => loadFromStorage(DATA_KEYS.customers, []));
  const [employees, setEmployees] = useState(() => loadFromStorage(DATA_KEYS.employees, []));
  const [sales, setSales] = useState(() => loadFromStorage(DATA_KEYS.sales, []));
  const [pos, setPos] = useState(() => loadFromStorage(DATA_KEYS.pos, []));
  const [settings, setSettings] = useState(() => loadFromStorage(DATA_KEYS.settings, SETTINGS_DEFAULT));
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [notifOpen, setNotifOpen] = useState(false);

  // Auto-save to localStorage whenever data changes
  useEffect(() => { saveToStorage(DATA_KEYS.medicines, medicines); }, [medicines]);
  useEffect(() => { saveToStorage(DATA_KEYS.suppliers, suppliers); }, [suppliers]);
  useEffect(() => { saveToStorage(DATA_KEYS.customers, customers); }, [customers]);
  useEffect(() => { saveToStorage(DATA_KEYS.employees, employees); }, [employees]);
  useEffect(() => { saveToStorage(DATA_KEYS.sales, sales); }, [sales]);
  useEffect(() => { saveToStorage(DATA_KEYS.pos, pos); }, [pos]);
  ...
  const alerts = [
    ...
    ...medicines.filter(m => {
      const d = daysUntilExpiry(m.expiry);
      return d > 0 && d < 60;
    }).map(m => ({ type: "danger", msg: `${m.name} — expires ${m.expiry}` })),
  ];
 ...
 const exportBackup = () => {
  ...
  a.href = "data:text/plain;charset=utf-8," + encodeURIComponent(encrypted);
  a.download = `medstore_backup_${today()}.encrypted`;
  a.click();
};
...
const importBackup = (e) => {
  ...
  reader.onload = (ev) => {
    try {
      ...
      // Restore data to state (will be encrypted again in localStorage)
      ...
      alert("Invalid or corrupted backup file! Wrong password or file damaged.");
    }
  };
  reader.readAsText(file);
};
  const initials = (settings.owner || "AD").split(" ").map(n => n[0]).join("").slice(0, 2).toUpperCase();

  const handleLogin = () => {
   // saveToStorage(DATA_KEYS.loggedIn, true);
    setIsLoggedIn(true);
  };

  const handleLogout = () => {
    saveToStorage(DATA_KEYS.loggedIn, false);
    setIsLoggedIn(false);
    setTab("dashboard");
  };

  const content = () => {
    if (tab === "dashboard") return <Dashboard medicines={medicines} sales={sales} suppliers={suppliers} />;
    if (tab === "inventory") return <Inventory medicines={medicines} setMedicines={setMedicines} suppliers={suppliers} />;
    if (tab === "pos") return <POS medicines={medicines} setMedicines={setMedicines} sales={sales} setSales={setSales} customers={customers} settings={settings} />;
    if (tab === "sales") return <SalesHistory sales={sales} setSales={setSales} settings={settings} />;
    if (tab === "purchases") return <PurchaseOrders pos={pos} setPos={setPos} suppliers={suppliers} medicines={medicines} setMedicines={setMedicines} />;
    if (tab === "suppliers") return <Suppliers suppliers={suppliers} setSuppliers={setSuppliers} />;
    if (tab === "customers") return <Customers customers={customers} setCustomers={setCustomers} />;
    if (tab === "expiry") return <ExpiryTracker medicines={medicines} />;
    if (tab === "reports") return <Reports sales={sales} medicines={medicines} />;
    if (tab === "employees") return <Employees employees={employees} setEmployees={setEmployees} />;
    if (tab === "settings") return <AppSettings settings={settings} setSettings={setSettings} exportBackup={exportBackup} importBackup={importBackup} />;
    return null;
  };
  ...
  return (
    <>
      {!isLoggedIn && (
        <LoginScreen onLogin={handleLogin} ownerCode={settings.loginCode} />
      )}
        ...
        <div style={{ display: "flex", height: "100vh", background: C.bg, fontFamily: "'Segoe UI',system-ui,sans-serif", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ height: 56, background: "#fff", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", padding: "0 20px", gap: 12, flexShrink: 0 }}>
              ...
              <div style={{ width: 34, height: 34, borderRadius: "50%", background: C.primaryBg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: C.primary, cursor: "pointer" }}>
                {initials}
              </div>
